  ...
  const config: WEBPACK.Configuration = {
    ...
    output: {
      ...
      assetModuleFilename: (pathData) => {
        ...
        let pathDir = PATH.dirname(pathData.filename ?? '').split('/');
        outputDir.forEach((v, i) => {
          if (pathDir[i] == v) {
            pathDir[i] = '';
          } else {
            pathDir.unshift('..');
        ...
        pathDir = pathDir.filter((v) => v != '');
        return `${pathDir.join('/')}/[name][ext][query]`;
      },
    },
    resolve: {
      extensions: ['.ts', '.js'],
    ...
    performance: {
      maxAssetSize: 1000 * KB, // 1MB以上のアセットを警告
      maxEntrypointSize: 1000 * KB, // アセットの合計は1MBまで許可
    },
    module: {
      rules: [
        {
        ...
        {
          test: /\.(woff|woff2|eot|ttf|svg|png|jpe?g|gif|webp)$/i,
          type: 'asset',
          parser: {
            dataUrlCondition: {
              maxSize: 5 * KB, // 5KBまで
